...
void TFT_22_ILI9225::begin() {
	...
	assertRST(0);
	delay(10);
	assertRST(1); // Pull the reset pin high to release the ILI9225C from the reset status
	delay(10);
...
void TFT_22_ILI9225::_setWindow(uint16_t x0, uint16_t y0, uint16_t x1,
		uint16_t y1) {
	_setWindow(x0, y0, x1, y1, TopDown_L2R); // default for drawing characters
}
void TFT_22_ILI9225::_setWindow(uint16_t x0, uint16_t y0, uint16_t x1,
		uint16_t y1, autoIncMode_t mode) {
	DB_PRINT("setWindows( x0=%d, y0=%d, x1=%d, y1=%d, mode=%d", x0, y0, x1, y1,
			mode);
	...
	DB_PRINT("gedreht: x0=%d, y0=%d, x1=%d, y1=%d, mode=%d", x0, y0, x1, y1,
			mode);
...
void TFT_22_ILI9225::drawRectangle(uint16_t x1, uint16_t y1, uint16_t x2,
...
void TFT_22_ILI9225::fillRectangle(uint16_t x1, uint16_t y1, uint16_t x2,
		uint16_t y2, uint16_t color) {
...
void TFT_22_ILI9225::drawCircle(uint16_t x0, uint16_t y0, uint16_t r,
		uint16_t color) {
...
void TFT_22_ILI9225::fillCircle(uint8_t x0, uint8_t y0, uint8_t radius,
		uint16_t color) {
...
void TFT_22_ILI9225::drawLine(uint16_t x1, uint16_t y1, uint16_t x2,
		uint16_t y2, uint16_t color) {
...
void TFT_22_ILI9225::splitColor(uint16_t rgb, uint8_t &red, uint8_t &green,
		uint8_t &blue) {
	// rgb16 = red5 green6 blue5
...
// Utilities
/*void TFT_22_ILI9225<RST,RS,CS,LED>::_writeCommand16(uint16_t command) {
 # ifdef HSPI_WRITE16
 SPI_DC_LOW();
 CS::low();
 ...
 void TFT_22_ILI9225<RST,RS,CS,LED>::_writeData16(uint16_t data) {
 # ifdef HSPI_WRITE16
 SPI_DC_HIGH();
 CS::low();
 HSPI_WRITE16(data);
...
void TFT_22_ILI9225::drawTriangle(uint16_t x1, uint16_t y1, uint16_t x2,
...
void TFT_22_ILI9225::fillTriangle(uint16_t x1, uint16_t y1, uint16_t x2,
		uint16_t y2, uint16_t x3, uint16_t y3, uint16_t color) {
	...
	int16_t dx11 = x2 - x1, dy11 = y2 - y1, dx12 = x3 - x1, dy12 = y3 - y1,
			dx22 = x3 - x2, dy22 = y3 - y2;
	int32_t sa = 0, sb = 0;
...
void TFT_22_ILI9225::setFont(uint8_t* font, bool monoSp) {
	...
	if (cfont.height % 8)
		cfont.nbrows++; // Set number of bytes used by height of font in multiples of 8
}
...
uint16_t TFT_22_ILI9225::drawChar(uint16_t x, uint16_t y, uint16_t ch,
		uint16_t color) {
	...
	if (cfont.monoSp)
		charWidth = cfont.width; // monospaced: get char width from font
	else
		charWidth = readFontByte(charOffset); // get chracter width from 1st byte
	...
	for (i = 0; i <= charWidth; i++) { // each font "column" (+1 blank column for spacing)
		...
		for (j = 0; j < cfont.nbrows; j++) {  // each column byte
			...
			for (uint8_t k = 0; k < 8; k++) {
				...
				if (fastMode) {
					uint16_t data = bitRead(charData, k) ? color : _bgColor;

					_writeData16(data);

				} else
					drawPixel(x + i, y + (j * 8) + k,
							bitRead(charData, k) ? color : _bgColor);
				h++;
			}
...
void TFT_22_ILI9225::drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap,
...
void TFT_22_ILI9225::drawBitmap(int16_t x, int16_t y, uint8_t *bitmap,
		int16_t w, int16_t h, uint16_t color) {
...
void TFT_22_ILI9225::drawXBitmap(int16_t x, int16_t y, const uint8_t *bitmap,
		int16_t w, int16_t h, uint16_t color) {
...
void TFT_22_ILI9225::_drawBitmap(int16_t x, int16_t y, const uint8_t *bitmap,
		int16_t w, int16_t h, uint16_t color, uint16_t bg, bool transparent,
		bool progmem, bool Xbit) {
		...
		for (i = 0; i < w; i++) {
			...
			if (x + i >= wx0 && x + i <= wx1) {
				...
				if (byte & maskBit) {
					...
					if (transparent)
						noAutoInc = true; // no autoincrement in transparent area!
					else
						_writeData16(bg);
				}
			}
...
//Usage: Export from GIMP to *.xbm, rename *.xbm to *.c and open in editor.
//C Array can be directly used with this function
/*void TFT_22_ILI9225<RST,RS,CS,LED>::drawXBitmap(int16_t x, int16_t y, const uint8_t *bitmap, int16_t w, int16_t h, uint16_t color) {
...
//High speed color bitmap
void TFT_22_ILI9225::drawBitmap(uint16_t x1, uint16_t y1, uint16_t** bitmap,
		int16_t w, int16_t h) {
	_setWindow(x1, y1, x1 + w, y1 + h);
...
// Draw a character
uint16_t TFT_22_ILI9225::drawGFXChar(int16_t x, int16_t y, unsigned char c,
		uint16_t color) {
...
void TFT_22_ILI9225::getGFXCharExtent(uint8_t c, int16_t *gw, int16_t *gh,
		int16_t *xa) {
