...
void XPT2046::begin(uint16_t width, uint16_t height) {
  ...
  cfg.pin_bit_mask = BIT(CONFIG_GPIO_TOUCH_CS);
  gpio_config(&cfg);
  gpio_set_level((gpio_num_t) CONFIG_GPIO_TOUCH_CS, 1);
...
uint16_t XPT2046::_readLoop(uint8_t ctrl, uint8_t max_samples) const {
  ...
  do {
    prev = cur;
    uint8_t out[2] = {0, ctrl};
    uint8_t rd[2] = {};
    spi_xfer(rd, out, 2);

    cur = ((rd[0]<<8) | rd[1])>>3;
    //printf("rd %d %x %x %x %x %d\n", ctrl, rd[0], rd[1], rd[2], rd[3], cur);

    //cur = (cur << 4) | ((uint16_t)rd[1] >> 4);  // 16 clocks -> 12-bits (zero-padded at end)
...
void XPT2046::getRaw (uint16_t &vi, uint16_t &vj, adc_ref_t mode, uint8_t max_samples) const {
  ...
  spi_beginTransaction(CONFIG_GPIO_TOUCH_CS);
  spi_addbuffer8(CTRL_HI_X | ctrl_lo);
  spi_flushdata();
  ...
  if (mode == MODE_DFR) {
    ...
    spi_addbuffer8(0);
    spi_addbuffer8(CTRL_HI_Y | CTRL_LO_SER);
  }
  spi_addbuffer8(0);
  spi_addbuffer8(0);
